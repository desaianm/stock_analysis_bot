...
Results cache to ``state/numeric_screen_cache.json`` for 24h. The cache key is
just the date — re-running the same day re-uses the snapshot, since per-stock
fundamentals don't move intraday in a way that affects screening.
"""
...
def fetch_snapshot(t: Ticker) -> StockSnapshot:
    ...
    try:
        ...
        if d2e is not None and d2e > 10:
            d2e = d2e / 100  # yfinance sometimes reports as percentage
...
def _read_cache() -> List[StockSnapshot]:
    payload = json.loads(CACHE_PATH.read_text(encoding="utf-8"))
    return [StockSnapshot.from_dict(d) for d in payload["snapshots"]]


def _write_cache(snaps: List[StockSnapshot]) -> None:
...
def apply_gates(snaps: Iterable[StockSnapshot], gates: ScreeningGates) -> List[StockSnapshot]:
    """Filter snapshots through the numeric gates. Missing values fail open
    (configurable per metric — see how the snapshot reasons about None below)."""
    ...
    for s in snaps:
        ...
        if gates.max_debt_equity is not None:
            if s.debt_to_equity is not None and s.debt_to_equity > gates.max_debt_equity:
                continue
        if gates.min_current_ratio is not None:
            if s.current_ratio is not None and s.current_ratio < gates.min_current_ratio:
                continue
